      ...
      <Stack.Screen options={{
        ...
        headerRight: () => (
          <Pressable onPress={handleDelete} hitSlop={12}>
            <Ionicons name="trash-outline" size={20} color={Colors.status.expired} />
          </Pressable>
        ),
      }} />
              ...
              <View style={styles.barcodeVisual}>
                {Array.from({ length: 28 }).map((_, i) => (
                  <View key={i} style={[styles.bar, {
                    width: ((voucher.code.charCodeAt(i % voucher.code.length) % 3) + 1) * 2,
...
const styles = StyleSheet.create({
  ...
  barcodeVisual: { flexDirection: 'row', alignItems: 'stretch', height: 72, gap: 1 },
  bar: { height: '100%', backgroundColor: '#1a1a1a', borderRadius: 1 },
